...
function judgeKidsDmRisk(signals) {
  ...
  const WEIGHTS = {
    K4:3, K7:3, K8:3, K9:3, K12:3, K14:3,
    K5:2, K6:2, K10:2, K11:2, K15:2, K16:2, K17:2, K19:2,
    K1:1, K2:1, K3:1, K13:1, K18:1,
  };
...
function showModal(handle, onSubmit, isKidsMode = false) {
  ...
  function refreshRiskMeter() {
    ...
    const result = isKidsMode ? judgeKidsDmRisk(signals) : judgeDmRisk(signals);
  ...
  submitBtn.addEventListener('click', () => {
    ...
    const localResult = isKidsMode ? judgeKidsDmRisk(signals) : judgeDmRisk(signals);
    const riskLabel = { BLOCK: '即遮断条件に該当', CRITICAL: '極めて高リスク', HIGH: '高リスク', MEDIUM: '中程度のリスク', LOW: '低リスク' };
    let localNote = '';
    if (isKidsMode) {
      // キッズモードはチェックなしでも必ず渡すにゃ
